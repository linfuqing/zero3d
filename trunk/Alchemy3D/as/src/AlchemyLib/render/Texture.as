...
//			Library.memory.writeBytes( byte );
		...
		public function setPixels(rect:Rectangle, bitmapData:BitmapData, destPoint:Point = null, level:uint = 0):void
		{
			if(!rect || !bitmapData)
				return;
			...
			var dataPosition:uint = destPoint ? (destPoint.x + destPoint.y * bitmapData.rect.width) : 0;
			var memeryPosition:uint;
			
			rect = rect.clone();
			
			if(bitmapData.rect.width < rect.right)
				rect.width -= rect.right - bitmapData.rect.width;
			
			if(bitmapData.rect.height < rect.bottom)
				rect.height -= rect.bottom - bitmapData.rect.height;
			...
			if(rect.x == 0 && rect.width == width)
			{
				...
				data.position = 0;
				Library.memory.writeBytes( data, dataPosition, Math.min(data.length, width * height) );
			}
			else
			{
