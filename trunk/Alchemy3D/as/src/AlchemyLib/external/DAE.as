package AlchemyLib.external
{
    ...
    */
    public class DAE extends AnimationObject
    {
		...
		public function get texturesFileName():Array
		{
			return textureFileName;
		}
		
		public function get textureMap():Array
		{
			return textureList;
		}
		...
		{	
			...
			if (String(node["instance_controller"].@url) == "" && String(node["instance_geometry"].@url) == "")
			{
...
//						} else {
		...
		private function dealNArrayNum(orinAry:Array):Array
		{
			var realList:Array = new Array;
			
			for(var v:uint = 0; v<orinAry.length; ++v)
			{
				var tmpStr:String = orinAry[v];
				var lastIndex:int = tmpStr.indexOf("\n");
				if(lastIndex != -1)
				{
					realList.push( Number( tmpStr.substring(0, lastIndex) ) );
					realList.push( Number( tmpStr.substring(lastIndex, tmpStr.length) ) );	
				}
				else
					realList.push( Number(orinAry[v]));
			}
			
			trace( realList.toString() );
			
			return realList;
		}
		
		private function dealNArrayStr(orinAry:Array):Array
		{
			var realList:Array = new Array;
			...
			for(var v:uint = 0; v<orinAry.length; ++v)
			{
				...
				if(lastIndex != -1)
				{
					realList.push( tmpStr.substring(0, lastIndex) );
					realList.push( tmpStr.substring(lastIndex+1, tmpStr.length) );	
				}
				else
					realList.push( orinAry[v] );
			}
		...
		private function parseMesh(name:String, geometry:XML, controller:XML, parent:Entity):void
		{
			...
			// Triangles
			var trianglesXMLList:XMLList = geometry["mesh"].polygons;
...
//				trianglesXMLList = geometry["mesh"].polylist;
			...
			for each (var triangles:XML in trianglesXMLList)
			{
				// Input
				var field:Array = [];
				var pXMLS:XMLList = triangles["p"];
				var data:Array  = new Array;
				
				for each(var pXML:XML in pXMLS)
					data = data.concat( pXML.toString().split(' ') );
			...
			var float_array:Vector.<Number> = getArray2(skin["source"].(@id == bindMatrixId)[0].float_array.toString());
...
//					Debug.warning("no joint found for " + name);
		...
		private function parseVertices(input:XML, geo:XML, output:Vector.<Vertex3D>):void
		{
			...
			if (acc != new XMLList())
			{
				...
				var floats:Vector.<Number>   = getArray2(floStr);
				var float:Number;
				// Build params array
				var params:Array = [];
				var param:String;
		...
		private function parseAnimationClips() : void
		{
			...
			var curAnimations:XMLList = anims["animation"];
			var animInfo:XML;
			if(curAnimations.length() > 0)
			{
				for each (var animation:XML in curAnimations)
				{
					animInfo = animation["animation"][0];
					...
					_channels = animInfo["channel"];
					if (_channels.length() == 1)
						parseChannel(animInfo, String(animation.@id), 0, _channelName);
					else
					{
						for (channelIndex = 0 ; channelIndex < _channels.length() ; channelIndex++)
						{
							...
							parseChannel(animInfo,String(animation.@id), channelIndex, _channelName + "_subAnim_" + type);
						}
					}
				}
			}
...
//			}
		...
		private function parseChannel(animation:XML, animID:String, channelIndex:int, channelName:String) : void
		{
			...
			if (animID.length > 0 && (type.split(".").length == 1 || type.split(".")[1].length > 1)) 
			{
				type = type.split(".")[0];
				
				if ((type == "image" || animID.split(".")[1] == "frameExtension")) 
				{
			...
			for each (var input:XML in sampler["input"])
			{
				...
				var semantic:String = input.@semantic;
				var aryStr:String = String(src["float_array"]);
				...
				switch(semantic) {
					...
					//	list = aryStr.split(' ');
						//此处第一帧不再播放	
						list = aryStr.split(' ');
						realList = dealNArrayNum(list);
						for(var v:uint = 1; v<realList.length; ++v)
							times.push(Number(realList[v]));
					...
					case "OUTPUT":
						...
						if (stride == 16)
						{
							...
							{			//i < len
								var m:Matrix3D = new Matrix3D();
								m.rawData = array2matrix(Vector.<Number>( realList.slice(nIndex*stride, nIndex*stride + 16) ), yUp, _scaling);
						...
						else
						{
							...
							{			//i < len
								...
								while (j < stride) 
								{
									params[i].push(Number(realList[i*stride + j]));
									++j;
								}
								++i;
							}
						...
						break;
					case "INTERPOLATION":
						list = String(src["Name_array"]).split(" ");
						realList = dealNArrayStr(list);
...
//						break;
		...
		private function getArray2(spaced:String):Vector.<Number>
		{
			spaced = spaced.split("\n").join(" ");
