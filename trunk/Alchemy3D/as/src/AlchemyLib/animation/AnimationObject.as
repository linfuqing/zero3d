package AlchemyLib.animation
{
	...
	public class AnimationObject extends Entity
	{
		...
		public function uninstallSkeletalAnimation(node:Entity = null):void
		{
			if(!node)
				node = this;
			
			var child:Entity;
			var length:uint = child.numChildren;
			for(var i:uint = 0; i < length; i ++)
			{
				( child = node.getChildByIndex(i) ).skeletalAnimation = null;
				
				uninstallSkeletalAnimation(child);
			}
		}
