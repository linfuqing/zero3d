...
int md2_read( UCHAR ** buffer, MD2 * m, Material * material, Texture * texture, int render_mode, unsigned int fps )
{
	...
	morphChannel_reBuild(m->entity->morphChannel, m->entity->animation, frames, m->header.num_frames, m->header.num_frames * duration);

	morphChannel_updateToFrame(m->entity->morphChannel, 0, m->entity->mesh);
...
void md2_destroyChannel(MorphChannel **channel)
{
	int i, length = (*channel)->length;
	for(i = 0; i < length; i ++)
		free( (*channel)->frames[i].vertices);

	free( (*channel)->frames );

	morphChannel_destroy(channel);
}
