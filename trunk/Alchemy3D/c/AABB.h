...
INLINE void aabb_empty(AABB * aabb)
{
	...
	aabb->max->x = aabb->max->y = aabb->max->z = aabb->max->w = -FLT_MAX;
}
...
INLINE void aabb_union(AABB* output, AABB* aabb1, AABB* aabb2)
{
	output->max->x = MAX(aabb1->max->x, aabb2->max->x);
	output->max->y = MAX(aabb1->max->y, aabb2->max->y);
	output->max->z = MAX(aabb1->max->z, aabb2->max->z);

	output->min->x = MIN(aabb1->min->x, aabb2->min->x);
	output->min->y = MIN(aabb1->min->y, aabb2->min->y);
	output->min->z = MIN(aabb1->min->z, aabb2->min->z);
}
...
AABB * aabb_setToTranslationBox(AABB * aabb, AABB * box, Vector3D* translation)
{
	vector3D_add(aabb->min, box->min, translation);
	vector3D_add(aabb->max, box->max, translation);
...
AABB * aabb_setToTransformedBox_CVV(AABB * aabb, AABB * box, Matrix4x4 * m)
{
	...
	if (m->m14 > 0.0f)
	{
		aabb->min->w += m->m14 * box->min->x;
		aabb->max->w += m->m14 * box->max->x;
	}
	else
	{
		aabb->min->w += m->m14 * box->max->x;
		aabb->max->w += m->m14 * box->min->x;
	}
	...
	if (m->m24 > 0.0f)
	{
		aabb->min->w += m->m24 * box->min->y;
		aabb->max->w += m->m24 * box->max->y;
	}
	else
	{
		aabb->min->w += m->m24 * box->max->y;
		aabb->max->w += m->m24 * box->min->y;
	}
	...
	if (m->m34 > 0.0f)
	{
		aabb->min->w += m->m34 * box->min->z;
		aabb->max->w += m->m34 * box->max->z;
	}
	else
	{
		aabb->min->w += m->m34 * box->max->z;
