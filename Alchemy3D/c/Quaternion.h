...
INLINE Vector3D* quaternion_toEuler(Vector3D* output, Quaternion* q )
{
	float test = q->x * q->y + q->z * q->w, sqx, sqy, sqz;

	if ( test > 0.5f ) {
		output->x = 2.0f * atan2f( q->x,q->w );
		output->y = HALF_PI;
		output->z = 0.0f;
	}
	else if ( test < -0.5f ) {
	...
	else
	{
		...
		output->x = atan2f( 2.0f * q->y * q->w - 2.0f * q->x * q->z, 1 - 2.0f * sqy - 2.0f * sqz );
		output->y = asinf( 2.0f * test );   
		output->z = atan2f( 2.0f * q->x * q->w - 2.0f * q->y * q->z, 1 - 2.0f * sqx - 2.0f * sqz );
	}
