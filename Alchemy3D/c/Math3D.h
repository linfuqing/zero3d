...
#define A32RGB24(a, rgb) ( (a) | ( (rgb) & 0xffffff) )
#define A8RGB24(a, rgb) A32RGB24( (a) << 24, rgb )
...
int buildAlphaTable(BYTE** alphaTable)
{
	int alphaLevel;
	int index;
	//BYTE* buffer;
	...
	if( ( *alphaTable = (BYTE*)malloc(sizeof(BYTE) * 16777216) ) == NULL ) 
		exit( TRUE );

	deltaAlpha = EPSILON_E6 + 1 / 255.0f;

	alpha = 0;

	for ( alphaLevel = 0; alphaLevel < 256; alphaLevel ++ )
	{
		//alphaTable[alphaLevel] = buffer;

		for ( index = 0; index < 65536; index ++ )
		{
			src1 = (float)(index & 0xff);

			src2 = (float)(index >> FIXP8_SHIFT);

			/*alphaTable[alphaLevel][index]*/(*alphaTable)[(alphaLevel << 16) | index] = (BYTE)(alpha * src1 + (1.0f - alpha) * src2);
