...
void viewport_destroy(Viewport **viewport)
{
	if(*viewport)
	{
		...
		renderList_destroy(&(*viewport)->renderList);
		renderList_destroy(&(*viewport)->transList);

		aabb_destroy(&(*viewport)->aabb);
...
void viewport_updateBeforeRender( Viewport * viewport )
{
	...
	if ( viewport->scene->fog && ( ! viewport->scene->fog->ready || viewport->camera->fnfDirty ) )
	{
		...
		if(!alphaTable/*viewport->useAlphaTable*/)
		{
...
void viewport_render( Viewport * viewport )
{
	...
	if(rl && rl->polygon && !alphaTable/*viewport->useAlphaTable*/)
	{
		buildAlphaTable(&alphaTable);
