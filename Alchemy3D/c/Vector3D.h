...
INLINE void vector3D_directionToRotation(float* x, float* y, Vector3D *direction)
{
	...
	{
		*x = - RAD2DEG( asin( direction->y / vector3D_length(direction) ) );
		*y =   0;
	}
	else if(direction->z < 0)
	{
		*x = - RAD2DEG( asin( direction->y / vector3D_length(direction) ) );
		*y =   RAD2DEG( PI - asin( direction->x / sqrt(direction->x * direction->x + direction->z * direction->z) ) );
	}
	else
	{
		*x = - RAD2DEG( asin( direction->y / vector3D_length(direction) ) );
		*y =   RAD2DEG( asin( direction->x / sqrt(direction->x * direction->x + direction->z * direction->z) ) );
	}
}
