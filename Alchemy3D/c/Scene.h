...
void scene_updateAfterRender( Scene * scene )
{
	...
	while( NULL != sceneNode )
	{
		...
		if ( sceneNode->entity->mesh )
		{
			...
			sceneNode->entity->transformDirty = FALSE;
			sceneNode->entity->mesh->v_dirty = FALSE;
			sceneNode->entity->mesh->texTransformDirty = FALSE;
			sceneNode->entity->mesh->nRenderList = 0;
			sceneNode->entity->mesh->nCullList = 0;
			sceneNode->entity->mesh->nClippList = 0;
		}
