package AlchemyLib.geom
{
	...
	public class Mesh3D extends Pointer
	{
		...
		public function getAABB(min:Vector3D, max:Vector3D, add:Boolean = false):void
		{
			if(min)
			{
				Library.memory.position = aabbPtr;
				Library.memory.position = Library.memory.readUnsignedInt();
				
				if(add)
				{
					min.x = Math.min( min.x, Library.memory.readFloat() );
					min.y = Math.min( min.y, Library.memory.readFloat() );
					min.z = Math.min( min.z, Library.memory.readFloat() );
				}
				else
				{
			...
			if(max)
			{
				Library.memory.position = aabbPtr + Library.intTypeSize;
				Library.memory.position = Library.memory.readUnsignedInt();
				
				if(add)
				{
					max.x = Math.max( max.x, Library.memory.readFloat() );
					max.y = Math.max( max.y, Library.memory.readFloat() );
					max.z = Math.max( max.z, Library.memory.readFloat() );
				}
				else
				{
		...
		private function setPointers(ps:Array):void
		{
			...
			if( ps[15] && _vertices )
			{
				...
				Library.alchemy3DLib.freeTmpBuffer( ps[15] );
			}
		}
