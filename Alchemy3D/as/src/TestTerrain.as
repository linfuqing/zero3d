package
{
	...
	public class TestTerrain extends Basic
	{
		...
		public function TestTerrain(viewWidth:int=640, viewHeight:int=480, fov:Number=50, near:Number=100, far:Number=5000)
		{
			...
			var fog:Fog = new Fog(new ColorTransform(1, 1, 1, 1), 800, 3000, 1, Fog.FOG_LINEAR);
			scene.addFog(fog);
			...
			camera.eye.setLocalAABB( new Vector3D(0, 100, 0), new Vector3D(1, 500, 0) );
			...
			var m:Material = new Material(null,
				new ColorTransform(0.2, 0.2, 0.2),
				new ColorTransform(0.2, 0.2, 0.2),
				new ColorTransform(0.2, 0.2, 0.2),
				new ColorTransform(0, 0, 0) );
			
			water = new Primitives(m, new Texture(null, ( (new waterTexture) as Bitmap ).bitmapData), RenderMode.RENDER_TEXTRUED_TRIANGLE_INVZB_ALPHA_32);
			water.toPlane(10000, 10000, 100, 100);
			...
			terrain = new MeshTerrain(null, m, new Texture(null, ( (new terrainTexture) as Bitmap ).bitmapData), RenderMode.RENDER_TEXTRUED_TRIANGLE_FOG_GSINVZB_ALPHA_32);
		...
		private function onKeyUp(e:KeyboardEvent):void
		{
			switch(e.keyCode)
			{
				...
				case Keyboard.SPACE:
					switch(renderMode)
					{
						case RenderMode.RENDER_TEXTRUED_TRIANGLE_FOG_GSINVZB_ALPHA_32:
						case RenderMode.RENDER_TEXTRUED_TRIANGLE_INVZB_32:
							terrain.mesh.setAttribute(Mesh3D.RENDERMODE_KEY, renderMode = RenderMode.RENDER_GOURAUD_TRIANGLE_INVZB_32);
							break;
						
						case RenderMode.RENDER_GOURAUD_TRIANGLE_INVZB_32:
							terrain.mesh.setAttribute(Mesh3D.RENDERMODE_KEY, renderMode = RenderMode.RENDER_FLAT_TRIANGLE_INVZB_32);
							break;
				...
				case Keyboard.CONTROL:
					if(enbleFog)
					{
						...
						if(renderMode == RenderMode.RENDER_TEXTRUED_TRIANGLE_FOG_GSINVZB_ALPHA_32)
							terrain.mesh.setAttribute(Mesh3D.RENDERMODE_KEY, renderMode = RenderMode.RENDER_TEXTRUED_TRIANGLE_INVZB_32);
					}
					else
					{
						enbleFog = true;
						
						if(renderMode == RenderMode.RENDER_TEXTRUED_TRIANGLE_INVZB_32)
							terrain.mesh.setAttribute(Mesh3D.RENDERMODE_KEY, renderMode = RenderMode.RENDER_TEXTRUED_TRIANGLE_FOG_GSINVZB_ALPHA_32);
					}
			}
		}
		...
		protected override function onRenderTick(e:Event=null):void
		{
			...
			//water.y = camera.y;
			camera.target = dae.worldPosition;
			//camera.rotationY ++;
		}
